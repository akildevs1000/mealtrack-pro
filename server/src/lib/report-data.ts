...
export async function fetchReportData(
  ...
  if (type === "consumption") {
    ...
    return {
      ...
      subtitle,
      columns: ["Camp", "Site", "Breakfast", "Lunch", "Dinner", "Total Served", "Estimated", "Variance"],
      rows,
    };
  }
  ...
  if (type === "camp") {
    ...
    const rows = camps.map((c) => {
      ...
      const duplicates =
        scanGroups.find((g) => g.campCode === c.code && g.status === "AlreadyServed")?._count._all ?? 0;
      const estimated = Math.round(c.employees * days * 0.85);
      const dev = devices.filter((d) => d.campCode === c.code);
      const coverage = estimated > 0 ? Math.round((served / estimated) * 100) : 0;
      return [
        c.code, c.name, c.site, c.employees, served, `${coverage}%`,
        Math.max(0, estimated - served), duplicates,
    ...
    return {
      ...
      subtitle,
      columns: ["Code", "Name", "Site", "Employees", "Served", "Coverage %", "Balance", "Duplicates", "Online", "Devices"],
      rows,
    };
  }
  ...
  // employee — synthesised, matches the /reports/employees logic
  const camps = await prisma.camp.findMany({ orderBy: { code: "asc" } });
  ...
  for (const c of camps) {
    ...
    for (let i = 0; i < per; i++) {
      ...
      rows.push([
        labourId, name, c.code,
        COMPANIES[(h >> 5) % COMPANIES.length],
        DESIGNATIONS[(h >> 7) % DESIGNATIONS.length],
        status,
        (h % 7) !== 0 ? "Yes" : "No",
        "Yes",
        (h % 5) !== 0 ? "Yes" : "No",
  ...
  return {
    ...
    columns: ["Labour ID", "Name", "Camp", "Company", "Designation", "Status", "Breakfast", "Lunch", "Dinner"],
    rows,
  };
}
...
export async function fetchTypedReportData(
  ...
  if (type === "consumption") {
    const camps = await prisma.camp.findMany({ where: campWhere(filters), orderBy: { code: "asc" } });
    const groups = await prisma.scan.groupBy({
      by: ["campCode", "meal"],
    ...
    const rows: ReportConsumptionRow[] = camps.map((c) => {
      ...
      const estimated = Math.round(c.employees * days * 0.85);
      return {
        code: c.code, name: c.name, site: c.site, employees: c.employees,
        breakfast, lunch, dinner, served, estimated, variance: served - estimated,
      };
    });
  ...
  if (type === "camp") {
    const [camps, scanGroups, devices] = await Promise.all([
      ...
      prisma.device.findMany({
        where: filters.campCodes && filters.campCodes.length > 0
  ...
  if (type === "wastage") {
    ...
    const rows: ReportWastageRow[] = camps.map((c) => {
      ...
      return {
        code: c.code, name: c.name, site: c.site,
        estimated, served: s, wastage, pct, status,
      };
    });
  ...
  for (const c of camps) {
    ...
    for (let i = 0; i < per; i++) {
      ...
      const nameBase = NAMES[h % NAMES.length];
      const name = i >= NAMES.length ? `${nameBase} ${Math.floor(i / NAMES.length) + 1}` : nameBase;
      const status = STATUSES[(h >> 3) % STATUSES.length] as ReportEmployeeRow["status"];
      if (filters.status && filters.status !== "all" && status !== filters.status) continue;
      const labourId = `LB-${(20000 + (h % 79999)).toString().padStart(5, "0")}`;
      if (qLower && !name.toLowerCase().includes(qLower) && !labourId.toLowerCase().includes(qLower)) continue;
      erows.push({
        labourId, name, camp: c.code,
