...
const createSchema = z.object({
  ...
  distributorEmployeeId: z.string().nullable().optional(),
    ...
    const result = await prisma.$transaction(async (tx) => {
      const m = await tx.campManager.create({
        data: {
          ...
          distributorEmployeeId: body.distributorEmployeeId ?? null,
          role: body.role,
          shift: body.shift,
...
function toApi(m: any) {
  return {
    ...
    distributorEmployeeId: m.distributorEmployeeId ?? null,
