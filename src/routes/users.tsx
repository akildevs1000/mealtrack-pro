...
const ROLES: { key: Role; label: string; tone: string }[] = [
  ...
  { key: "manager", label: "Distributor", tone: "bg-amber-500/10 text-amber-500 border-amber-500/20" },
];
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage software operators, users and distributors. Assign per-tab View / Edit / Delete permissions.
          </p>
        </div>
        {tab === "users" && canManage && (
        ...
        <div key={role.key} className="rounded-2xl border border-border bg-card overflow-hidden">
          <div className="px-5 py-3 border-b border-border bg-secondary/30 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">{role.key === "manager" ? "Distributor — sees only their assigned camp" : ""}</span>
            </div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
...
function UserDialog({ initial, onClose, onSave }: {
  ...
  function submit(e: React.FormEvent) {
    ...
    if (form.role === "manager" && !form.assignedCampCode) {
      alert("Distributors must be assigned a camp.");
      return;
    }
    onSave(form);
  }
      ...
      <form onClick={(e) => e.stopPropagation()} onSubmit={submit}
        ...
        <div className="px-6 py-4 border-b border-border flex items-center justify-between">
          <div>
            ...
            <div className="text-xs text-muted-foreground">{isEdit ? `Updating ${initial.username}` : "Create a software operator, user or distributor"}</div>
          </div>
