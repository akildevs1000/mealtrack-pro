...
import { Coffee, Sun, Moon, TrendingUp, Utensils, Building2, CalendarRange, CalendarDays, RotateCcw, Save, Pencil } from "lucide-react";
...
type Row = { key: string; label: string; sub?: string; breakfast: number; lunch: number; dinner: number };
...
function defaultsFor(camp: typeof camps[number]): Record<string, { breakfast: number; lunch: number; dinner: number }> {
  // Per-weekday baseline scaled to this camp's labour count
  const base = camp.employees;
  return Object.fromEntries(WEEKDAYS.map((d, i) => {
    const isFri = d === "Fri", isSun = d === "Sun";
...
function Forecast() {
  ...
  const [selectedCamp, setSelectedCamp] = useState<string>(visibleCamps[0]?.code ?? camps[0].code);
  useEffect(() => {
    if (!visibleCamps.some((c) => c.code === selectedCamp)) {
      setSelectedCamp(visibleCamps[0]?.code ?? camps[0].code);
    }
  }, [visibleCamps, selectedCamp]);
  const camp = visibleCamps.find((c) => c.code === selectedCamp) ?? visibleCamps[0] ?? camps[0];
  ...
  const [store, setStore] = useState<Record<string, CampStore>>(() => {
    const out: Record<string, CampStore> = {};
    for (const c of camps) out[c.code] = { weekday: defaultsFor(c), dates: {} };
    return out;
  });

  // Make sure new camps get defaults
  useEffect(() => {
    setStore((prev) => {
      if (prev[camp.code]) return prev;
      return { ...prev, [camp.code]: { weekday: defaultsFor(camp), dates: {} } };
    });
  }, [camp]);

  const campStore = store[camp.code] ?? { weekday: defaultsFor(camp), dates: {} };

  const rows: Row[] = useMemo(() => {
    if (mode === "weekday") {
      return WEEKDAYS.map((d) => ({
        key: d, label: d, sub: "Weekly template",
        ...campStore.weekday[d],
      }));
    }
    const list: Row[] = [];
    ...
    while (cur <= end && safety++ < 366) {
      ...
      list.push({
        key: iso, label: fmtDate(iso), sub: wd,
  ...
  function updateRow(key: string, meal: "breakfast" | "lunch" | "dinner", value: number) {
    ...
    setStore((prev) => {
      ...
      if (mode === "weekday") {
        return {
          ...prev,
          [camp.code]: { ...cur, weekday: { ...cur.weekday, [key]: { ...cur.weekday[key], [meal]: v } } },
        };
      }
  ...
  const totals = rows.reduce((a, r) => ({ b: a.b + r.breakfast, l: a.l + r.lunch, dn: a.dn + r.dinner }), { b: 0, l: 0, dn: 0 });
  const grand = totals.b + totals.l + totals.dn;
  const dailyAvg = rows.length ? Math.round(grand / rows.length) : 0;
  const peak = rows.length ? rows.reduce((p, r) => (r.breakfast + r.lunch + r.dinner > p.breakfast + p.lunch + p.dinner ? r : p)) : null;

  const chartData = rows.map((r) => ({ day: r.label, breakfast: r.breakfast, lunch: r.lunch, dinner: r.dinner }));

  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <div className="text-xs uppercase tracking-wider text-muted-foreground font-medium">Planning & Procurement</div>
          <h1 className="font-display text-2xl font-bold tracking-tight mt-1">Editable Meal Forecast</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={resetAll} title="Reset this camp to defaults"
      ...
      <div className="rounded-xl border border-border bg-card p-4 flex flex-wrap items-center gap-3">
        <div className="inline-flex rounded-lg border border-border bg-secondary/40 p-1">
          <ModeBtn active={mode === "weekday"} onClick={() => setMode("weekday")} icon={<CalendarDays className="size-3.5" />} label="Weekday template" />
          <ModeBtn active={mode === "range"} onClick={() => setMode("range")} icon={<CalendarRange className="size-3.5" />} label="Date range" />
        </div>
        {mode === "range" && (
          <>
            <DateField label="From" value={from} onChange={setFrom} />
            <DateField label="To" value={to} onChange={setTo} />
            <button onClick={applyTemplateToRange}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard label={mode === "weekday" ? "Weekly demand" : "Range demand"} value={grand.toLocaleString()} icon={Utensils} tone="primary" hint={camp.code} />
        <KpiCard label="Daily average" value={dailyAvg.toLocaleString()} icon={TrendingUp} tone="accent" hint="Meals per day" />
        <KpiCard label="Peak day" value={peak?.label ?? "—"} icon={Sun} tone="warm" hint={peak ? `${(peak.breakfast + peak.lunch + peak.dinner).toLocaleString()} meals` : ""} />
        <KpiCard label="Employees" value={camp.employees.toLocaleString()} icon={Building2} tone="muted" hint={camp.site} />
      </div>
      ...
      <div className="rounded-2xl border border-border bg-card p-6 shadow-card">
        <div className="mb-4">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">{mode === "weekday" ? "Weekly template" : `${fmtDate(from)} → ${fmtDate(to)}`}</p>
        </div>
        <div className="h-72">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
              <defs>
                <linearGradient id="fb" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="var(--chart-3)" stopOpacity={0.5} /><stop offset="100%" stopColor="var(--chart-3)" stopOpacity={0} /></linearGradient>
                <linearGradient id="fl" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="var(--chart-1)" stopOpacity={0.5} /><stop offset="100%" stopColor="var(--chart-1)" stopOpacity={0} /></linearGradient>
                <linearGradient id="fd" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="var(--chart-2)" stopOpacity={0.5} /><stop offset="100%" stopColor="var(--chart-2)" stopOpacity={0} /></linearGradient>
              </defs>
              ...
              <Area type="monotone" dataKey="breakfast" stroke="var(--chart-3)" fill="url(#fb)" strokeWidth={2} />
              <Area type="monotone" dataKey="lunch" stroke="var(--chart-1)" fill="url(#fl)" strokeWidth={2} />
              <Area type="monotone" dataKey="dinner" stroke="var(--chart-2)" fill="url(#fd)" strokeWidth={2} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-muted/40 text-xs uppercase tracking-wider text-muted-foreground">
              <tr>
                <th className="text-left px-6 py-3 font-medium">{mode === "weekday" ? "Weekday" : "Date"}</th>
            ...
            <tbody>
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.key} className={`border-t border-border transition-colors ${isSunday ? "bg-amber-500/10 hover:bg-amber-500/15" : "hover:bg-muted/30"}`}>
                    <td className="px-6 py-2.5">
                      <div className={`font-medium inline-flex items-center gap-2 ${isSunday ? "text-amber-600 dark:text-amber-400" : ""}`}>
                        {r.label}
                        {isSunday && <span className="text-[10px] uppercase tracking-wider px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-700 dark:text-amber-300 font-semibold">Sun</span>}
                      </div>
                      {r.sub && <div className="text-xs text-muted-foreground">{r.sub}</div>}
                    </td>
                    <NumCell value={r.breakfast} onChange={(v) => updateRow(r.key, "breakfast", v)} />
              ...
              {rows.length === 0 && (
                <tr><td colSpan={5} className="px-6 py-12 text-center text-muted-foreground">Pick a valid date range.</td></tr>
              )}
            </tbody>
            {rows.length > 0 && (
              <tfoot className="bg-secondary/40 text-sm font-semibold">
                <tr className="border-t border-border">
                  ...
                  <td className="px-4 py-3 text-right tabular-nums">{totals.dn.toLocaleString()}</td>
...
function MealTotalCard({ label, value, icon: Icon, color }: { label: string; value: number; icon: React.ComponentType<{ className?: string }>; color: string }) {
  return (
    <div className="rounded-2xl border border-border bg-card p-5 shadow-card">
      <div className="flex items-center justify-between">
        <div className="text-sm font-medium text-muted-foreground">{label}</div>
        <div className="size-9 rounded-xl grid place-items-center" style={{ background: `color-mix(in oklab, ${color} 18%, transparent)`, color }}>
